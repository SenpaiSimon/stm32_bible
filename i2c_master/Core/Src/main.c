...
int main(void)
{
  ...
  while (1)
  {
    ...
    ********************************************/
	status = HAL_I2C_Mem_Read(&hi2c1, DEVICE_ADDRESS, REGISTER_TO_READ, 1, buf, 2, 100);
     ...
     ********************************************/
	buf[0] = REGISTER_TO_READ;
	status = HAL_I2C_Master_Transmit(&hi2c1, DEVICE_ADDRESS, buf, 1, 100);

	if(status == HAL_OK) {
		status = HAL_I2C_Master_Receive(&hi2c1, DEVICE_ADDRESS, buf, 2, 100);
